...
@section('content')
    ...
    <div class="space-y-6">
        <div><h2 class="text-xl font-bold text-slate-950">Semua Presensi</h2><p class="mt-1 text-sm text-slate-500">Rekap daftar hadir manual seluruh kegiatan.</p></div>
        ...
        <div class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            <form method="GET" action="{{ route('attendances.index') }}" class="grid gap-3 sm:grid-cols-2 xl:grid-cols-[minmax(220px,1fr)_180px_200px_190px_auto]">
                <select name="activity_id" aria-label="Filter kegiatan" class="block w-full rounded-lg border-slate-300 shadow-sm focus:border-emerald-600 focus:ring-emerald-600">
                    <option value="">Semua kegiatan</option>
                    @foreach ($activities as $activity)<option value="{{ $activity->id }}" @selected((string) $activityId === (string) $activity->id)>{{ $activity->activity_date->format('d M Y') }} - {{ $activity->title }}</option>@endforeach
                </select>
                ...
                <select name="member_id" aria-label="Filter anggota" class="block w-full rounded-lg border-slate-300 shadow-sm focus:border-emerald-600 focus:ring-emerald-600">
                    <option value="">Semua anggota</option>
                    @foreach ($members as $member)<option value="{{ $member->id }}" @selected((string) $memberId === (string) $member->id)>{{ $member->full_name }}</option>@endforeach
                </select>
                <select name="department_id" aria-label="Filter bidang anggota" class="block w-full rounded-lg border-slate-300 shadow-sm focus:border-emerald-600 focus:ring-emerald-600">
                ...
                <div class="flex gap-2 sm:col-span-2 xl:col-span-1">
                    ...
                    <a href="{{ route('attendances.index') }}" class="flex flex-1 items-center justify-center rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-50">Reset</a>
                </div>
            </form>
        </div>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-200">
                    <thead class="bg-slate-50"><tr>
                        @foreach (['Kegiatan', 'Tanggal', 'Nama Anggota', 'Bidang', 'Status', 'Metode', 'Catatan'] as $heading)<th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-500">{{ $heading }}</th>@endforeach
                    ...
                    <tbody class="divide-y divide-slate-100">
                        @forelse ($attendances as $attendance)
                            <tr>
                                ...
                                <td class="whitespace-nowrap px-4 py-4 text-right"><a href="{{ route('attendances.edit', $attendance) }}" class="rounded-md px-2.5 py-1.5 text-sm font-semibold text-emerald-700 hover:bg-emerald-50">Edit</a></td>
                            </tr>
                        @empty
                            <tr><td colspan="8" class="px-4 py-10 text-center text-sm text-slate-500">Belum ada data presensi.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
