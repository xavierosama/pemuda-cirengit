...
@section('content')
    <div class="space-y-6">
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
                <h2 class="text-xl font-bold text-slate-950">Data Jabatan</h2>
                <p class="mt-1 text-sm text-slate-500">Kelola daftar jabatan organisasi Pemuda Persis Cirengit.</p>
            </div>
            <a href="{{ route('positions.create') }}" class="inline-flex items-center justify-center rounded-lg bg-emerald-700 px-4 py-2 text-sm font-semibold text-white transition hover:bg-emerald-800 focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:ring-offset-2">
                Tambah Jabatan
            </a>
        </div>
        ...
        <div class="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            <form method="GET" action="{{ route('positions.index') }}" class="grid gap-3 md:grid-cols-[1fr_220px_auto]">
                <div>
                    <label for="search" class="sr-only">Cari nama jabatan</label>
                    <input
                        id="search"
                        name="search"
                        type="search"
                ...
                <div class="flex gap-2">
                    ...
                    <a href="{{ route('positions.index') }}" class="inline-flex flex-1 items-center justify-center rounded-lg border border-slate-300 px-4 py-2 text-sm font-semibold text-slate-700 transition hover:bg-slate-50 md:flex-none">
                        Reset
                    </a>
                </div>
            </form>
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full divide-y divide-slate-200">
                    <thead class="bg-slate-50">
                        <tr>
                            <th class="px-4 py-3 text-left text-xs font-bold uppercase tracking-wide text-slate-500">Nama Jabatan</th>
                    ...
                    <tbody class="divide-y divide-slate-100 bg-white">
                        @forelse ($positions as $position)
                            <tr>
                                ...
                                <td class="px-4 py-4 text-sm text-slate-600">
                                    {{ str($position->description ?: '-')->limit(90) }}
                                </td>
                                ...
                                <td class="whitespace-nowrap px-4 py-4 text-right text-sm font-semibold">
                                    <div class="flex justify-end gap-2">
                                        <a href="{{ route('positions.show', $position) }}" class="rounded-md px-2.5 py-1.5 text-slate-700 transition hover:bg-slate-100">Detail</a>
                                        <a href="{{ route('positions.edit', $position) }}" class="rounded-md px-2.5 py-1.5 text-emerald-700 transition hover:bg-emerald-50">Edit</a>
                                        <form method="POST" action="{{ route('positions.destroy', $position) }}" onsubmit="return confirm('Hapus data jabatan ini?')">
                                            @csrf
                                            @method('DELETE')
                        ...
                        @empty
                            <tr>
                                <td colspan="4" class="px-4 py-10 text-center text-sm text-slate-500">Belum ada data jabatan.</td>
                            </tr>
                        @endforelse
                    </tbody>
                </table>
